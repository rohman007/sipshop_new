

<html>
<head></head>
<body>
    
<ul class="show-product-list dropdown shadow navbar-nav-scroll p-sm-3 p-3 pt-0 pt-sm-0" aria-labelledby="dropsearchdown">

<style type="text/css">
    .show-product-list {
        position: absolute;
        z-index: 1;
        margin-right: 0;
        width: 100%;
        background: rgb(122 122 122 / 90%);
        list-style: none;
        max-height:300px;
        border-radius: 10px;
        margin-top: 5px;
        overflow-y: auto !important;
    }
    @media(max-width:767px){
        .show-product-list{max-height:65vh}
    }
    .dropdown-item-text {
    	color: #dee2e6;
    }
    .show-product-list > li > a > .row:hover {
        background: #444;
    }
    span.mb-3 {
        margin-bottom: 5px !important;
    }
    .show-product-list > li > a > .row {
        align-items: center;
        color: #f5da36;
        padding: 10px 0px;
        border-radius: 7px;
    }
    .show-product-list > li > a > .row img {
        border-radius: 10px;
    }
    .dropdown-item{
        white-space: normal;
    }
</style>
		<li><span class="dropdown-item-text text-yellow fw-bold mb-1 mb-3 fs-6 mt-3" style=""><b>Game Mobile</b></span></li>
	<hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/genshin-impact" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16846614836216.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Genshin Impact</b></div>
                    </div>
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/pubg-mobile" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16866681903890.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>PUBG Mobile</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/revelation-:-infinite-journey" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16866694363604.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Revelation : Infinite Journey</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/never-after" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16867355143461.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Never After</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/idle-legends-:-gods-saga" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16869251103060.jpg" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Idle Legends : GODS SAGA</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/one-punch-man-:-the-strongest" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16867349449062.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>One Punch Man : The Strongest</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/project-entropy" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16867350232163.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Project Entropy</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/rise-of-kingdom" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16867350862288.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Rise of Kingdom</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/marvel-snap" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16867354856874.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Marvel Snap</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/omega-legends" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16866918984589.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Omega Legends</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/jade-legends" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16869249581196.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Jade Legends</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/mirage-:-perfect-skyline" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16869247938860.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Mirage : Perfect Skyline</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/super-sus" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16867349087205.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Super SUS</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/hay-day" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16867352963400.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Hay Day</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/conquer-online" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16869250515933.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Conquer Online</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/football-master-2" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16866695175921.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Football Master 2</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/goddess-of-victory:-nikke" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16867355628731.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Goddess of Victory: Nikke</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/dunk-city-dynasty" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16866914345203.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Dunk City Dynasty</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/clash-royale" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16867346911576.jpg" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Clash Royale</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/clash-of-clans" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16867347366515.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Clash of Clans</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/brawl-stars" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16867347795700.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Brawl Stars</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/heroes-evolved" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16869247462873.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Heroes Evolved</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/bleach-mobile-3d" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16869248842356.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>BLEACH Mobile 3D</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/echocalypse" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16869249266970.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Echocalypse</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/light-of-thel-:-new-era" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16866693915926.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Light of Thel : New Era</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/ensemble-stars-music" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16866694739347.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Ensemble Stars Music</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/laplace-m" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16865455273252.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Laplace M</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/sky:-children-of-the-light" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16867351895313.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Sky: Children of the Light</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/state-of-survival:-zombie-war" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16867352249277.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>State of Survival: Zombie War</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/miko-era-:-twelve-myths" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16869248502596.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Miko Era : Twelve Myths</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/call-of-duty-mobile" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16866679742019.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Call of Duty Mobile</b></div>
                    </div>
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/free-fire" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16846860969715.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Free Fire</b></div>
                    </div>
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/marvel-super-war" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16866692683808.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>MARVEL Super War</b></div>
                    </div>
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/arena-of-valor" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16866680457558.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Arena of Valor</b></div>
                    </div>
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/ragnarok-m-eternal-love" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16866681418816.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Ragnarok M : Eternal Love</b></div>
                    </div>
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/mobile-legends" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16951920116062.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Mobile Legends</b></div>
                    </div>
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/metal-slug-awakening" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16932899506245.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Metal Slug : Awakening</b></div>
                    </div>
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        	<li><span class="dropdown-item-text text-yellow fw-bold mb-1 mb-3 fs-6 mt-3" style=""><b>Game PC</b></span></li>
	<hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/luna-online-new-world" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16854775889733.jpg" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Luna Online New World</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/valorant" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16867348743494.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Valorant</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/eudemons-online" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16869252213758.jpg" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Eudemons Online</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        	<li><span class="dropdown-item-text text-yellow fw-bold mb-1 mb-3 fs-6 mt-3" style=""><b>Pulsa &amp; Data</b></span></li>
	<hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/axis" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16866721394607.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Axis</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/by-u" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16866727289327.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>By U</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/three" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16866723366101.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Three</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/telkomsel" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16866723691796.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Telkomsel</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/smartfren" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16866724332477.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Smartfren</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/xl" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16866726359713.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>XL</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/token-listrik-pln" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16869253492047.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Token Listrik PLN</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        	<li><span class="dropdown-item-text text-yellow fw-bold mb-1 mb-3 fs-6 mt-3" style=""><b>Streaming</b></span></li>
	<hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/amazon-prime-video" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16866696894699.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Amazon Prime Video</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/disney-hotstar" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16866697649282.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Disney Hotstar</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/vidio-premier" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16866698904741.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Vidio Premier</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/viu-premium" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16866701141455.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Viu Premium</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/wetv-premium" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16866700207556.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>WeTV Premium</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        <li>
        <a href="https://sipshop.co.id/order/youtube-premium" class="dropdown-item">
            <div class="row">
                <div class="col-3">
                    <img src="https://api.sipshop.co.id/images/16866699732517.png" style="" class="img-fluid">
                </div>
                <div class="col-9 ps-0">
                    <div class="row">
                        <div class="col-md-12"><b>Youtube Premium</b></div>
                    </div>
                    
                </div>
            </div>
        </a>
    </li>
    <hr class="dropdown-divider" style="">
        </ul>
</body>

</html>